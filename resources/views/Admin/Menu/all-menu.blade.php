...
<div class="container grid px-6 mx-auto">
    <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">Bảng Menu</h2>

    <div class="flex justify-between items-center mb-4">
        <h4 class="mb-0 text-lg font-semibold text-gray-600 dark:text-gray-300">Danh sách Menu</h4>
        <a href="{{ URL::to('Admin/add-menu') }}" style="display: inline-block; padding: 8px 16px; background-color: #0d6efd; color: #fff; border-radius: 4px; text-decoration: none;" title="Thêm mới">
    ...
    <div class="w-full mb-8 overflow-hidden rounded-lg shadow-xs">
            ...
            <table class="w-full whitespace-no-wrap">
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                    ...
                    <tr class="text-gray-700 dark:text-gray-400">
                        <td class="px-4 py-3 text-sm">{{ $menu->name }}</td>
                        <td class="px-4 py-3 text-sm">
                            @if($menu->image)
                                @if(Str::startsWith($menu->image, 'http'))
                                    <img src="{{ $menu->image }}" width="60" alt="Menu Image">
                                @else
                                   <img width="60" src="{{asset('public/backend/assets/img/menu/'. $menu->image) }}" alt="" loading="lazy" />
                                @endif
                            @else
                        ...
                        <td class="px-4 py-3 text-sm">
                            <i class="{{ $menu->status ? 'fas fa-toggle-on fa-2x text-primary' : 'fas fa-toggle-off fa-2x text-danger' }}"></i> <!-- Icon -->
                        </td>
                        <td class="px-4 py-3">
                            <!-- Nút chỉnh sửa -->
                            <a href="{{ URL::to('Admin/edit-menu/'. $menu->menu_id) }}" class="text-primary" title="Chỉnh sửa">
                                <i class="fas fa-edit"></i>
                            </a>
                            <!-- Nút xóa -->
                             <a href="{{ URL::to('delete-menu/'.$menu->menu_id) }}" onclick="return confirm('Bạn có chắc chắn muốn xóa?')" class="text-danger ml-2" title="Xóa">
        ...
        <div class="grid px-4 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
            <span class="flex items-center col-span-3">Showing 1-10 of {{ count($all_menu) }}</span>
